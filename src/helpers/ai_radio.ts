...
export const parseSharedShow = (text: string): SharedShow => {
  ...
  const version = safeInteger(String(raw.version ?? ""), 0, 0);
...
const sharedPlaysRule = (value: unknown): PlaysRule => {
  ...
  switch (value.kind) {
    ...
    case "every_n_songs":
    case "every_n_min":
      return { kind: value.kind, n: safeInteger(String(value.n ?? ""), 1, 1) };
    case "occasionally":
      return {
        kind: "occasionally",
        percent: Math.min(100, safeInteger(String(value.percent ?? ""), 1, 10)),
      };
    default:
      throw invalidImportError();
  }
...
const sharedSegment = (value: unknown): SharedShowSegment => {
  ...
  return {
    ...
    maxChars: safeInteger(String(value.maxChars ?? ""), 0, 0),
